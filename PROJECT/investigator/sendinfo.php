...
if(isset($_POST["binin"])){
    include '../admin/folders/policepdf.php';
    $active=$_POST["CLOSED"];
    $messo=$_POST['messo'];
    $assign=$_POST['assign'];
    ...
    //UPDATE STATUS
    $send= "UPDATE complaintpolice set PCOMPSTATUS= ? where POLICECOMPLAINTID= ? ";
     $chick = mysqli_stmt_init($conn);
     if(mysqli_stmt_prepare($chick, $send)){
        mysqli_stmt_bind_param($chick, "ss", $active,$pd);
...
// preparing attachment
    ...
    // preparing attachment
        ...
        $message2 .= "--{$mime_boundary}--";  
         ...
         if(mail($messo,$SUBJECT12,$message2,$HEADIE2)){
            header("location: ./indexpage.php?error=emailsent");
           
           }
           else{
               header("location: ./indexpage.php?error=emailnotsent1");
               exit();
            }
    ...
    if(mysqli_num_rows($dad_run)>0){
     ...
     if(mail($email2, $SUBJECT3,$MESSAGE3,$HEADIE1)){
        header("location: ./indexpage.php?error=sent");
       
       }
       else{
           header("location: ./policeassignment.php?error=emailnotsent");
           exit();
        }
