...
function IDEmailexists($conn, $ID, $emailaddress)  {
    ...
    $sql = "SELECT * FROM investigators WHERE InvestID = ? OR emailaddress = ?;";
...
function createofficer($conn, $ID, $firstname, $secondname, $phonenumber, $emailaddress, $passwrd, $photo,$position) {
   

    $sql = "INSERT INTO investigators(InvestID, Firstname, Secondname,phonenumber, emailaddress, passwrd, photo) VALUES(?,?,?,?,?,?,?)";
   //writing prepared statements to prevent the user from changing the code from the UI design infront or prevent any form of sql injection
    $stmt = mysqli_stmt_init($conn);
